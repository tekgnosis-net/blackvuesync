...
def test_to_recording(
    filename: str, expected_recording: _sync.Recording | None
) -> None:
    recording = _sync.to_recording(filename, "none")
...
def test_to_downloaded_recording(
    filename: str, expected_recording: _sync.DownloadedRecording | None
) -> None:
    recording = _sync.to_downloaded_recording(filename, "none")
...
def test_get_group_name(
    ...
    assert expected_daily_group_name == _sync.get_group_name(
        recording_datetime, "daily"
    )
    assert expected_weekly_group_name == _sync.get_group_name(
        recording_datetime, "weekly"
    )
    assert expected_monthly_group_name == _sync.get_group_name(
        recording_datetime, "monthly"
    )
    assert expected_yearly_group_name == _sync.get_group_name(
...
def test_calc_cutoff_date(keep: str, expected_cutoff_date: datetime.datetime) -> None:
    try:
        ...
        cutoff_date = _sync.calc_cutoff_date(keep)
...
def test_sort_recordings(
    ...
    recordings = [
        r for r in [_sync.to_recording(f, "none") for f in filenames] if r is not None
    ]
    expected_sorted_recordings = [
        r
        for r in [_sync.to_recording(f, "none") for f in expected_sorted_filenames]
        if r is not None
    ]

    # copy
    sorted_recordings = recordings.copy()
    _sync.sort_recordings(sorted_recordings, priority)
...
def test_parse_duration(duration: str, expected_timedelta: datetime.timedelta) -> None:
    assert expected_timedelta == _sync.parse_duration(duration)
...
class TestFailedMarker:
    ...
    def test_get_failed_marker_filepath_no_grouping(self) -> None:
        ...
        filepath = _sync.get_failed_marker_filepath(
    ...
    def test_mark_download_failed_creates_marker(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as dest:
            ...
            _sync.mark_download_failed(dest, None, filename)

            marker_filepath = _sync.get_failed_marker_filepath(dest, None, filename)
    ...
    def test_remove_download_failed_marker(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as dest:
            ...
            _sync.remove_download_failed_marker(dest, None, filename)
    ...
    def test_is_download_blocked_by_failure_recent_marker(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as dest:
            ...
            try:
                ...
                assert _sync.is_download_blocked_by_failure(dest, None, filename)
            finally:
    ...
    def test_is_download_blocked_by_failure_stale_marker(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as dest:
            ...
            try:
                ...
                assert not _sync.is_download_blocked_by_failure(dest, None, filename)
            finally:
                _sync.retry_failed_after = original
    ...
    def test_marker_in_grouping_directory(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as dest:
            ...
            _sync.mark_download_failed(dest, group_name, filename)

            marker_filepath = _sync.get_failed_marker_filepath(
            ...
            try:
                _sync.retry_failed_after = datetime.timedelta(days=1)
                assert _sync.is_download_blocked_by_failure(dest, group_name, filename)
            finally:
                _sync.retry_failed_after = original

            _sync.remove_download_failed_marker(dest, group_name, filename)
            assert not os.path.exists(marker_filepath)
...
def test_parse_filter(value: str, expected: tuple[str, ...]) -> None:
    assert _sync.parse_filter(value) == expected
...
def test_parse_filter_invalid(value: str) -> None:
    with pytest.raises(argparse.ArgumentTypeError):
        _sync.parse_filter(value)


def _recording(rec_type: str, direction: str) -> _sync.Recording:
    """creates a minimal Recording for filter tests"""
    return _sync.Recording(
...
class TestApplyRecordingFilters:
    ...
    def test_no_filters_returns_all(self) -> None:
        ...
        result = _sync.apply_recording_filters(recordings, None, None)
    ...
    def test_include_type_only(self) -> None:
        ...
        result = _sync.apply_recording_filters(recordings, ("N",), None)
    ...
    def test_include_type_and_direction(self) -> None:
        ...
        result = _sync.apply_recording_filters(recordings, ("NF",), None)
    ...
    def test_include_multiple_codes(self) -> None:
        ...
        result = _sync.apply_recording_filters(recordings, ("N", "PF"), None)
        assert result == [recordings[0], recordings[1]]
    ...
    def test_exclude_type_only(self) -> None:
        ...
        result = _sync.apply_recording_filters(recordings, None, ("P",))
    ...
    def test_exclude_type_and_direction(self) -> None:
        ...
        result = _sync.apply_recording_filters(recordings, None, ("NR",))
        assert result == [recordings[0]]
    ...
    def test_include_and_exclude_combined(self) -> None:
        ...
        result = _sync.apply_recording_filters(recordings, ("N",), ("NR",))
        assert result == [recordings[0]]

    def test_empty_result(self) -> None:
        recordings = [_recording("N", "F")]
        result = _sync.apply_recording_filters(recordings, ("P",), None)
        assert result == []

    def test_empty_include_returns_all(self) -> None:
        recordings = [_recording("N", "F"), _recording("P", "R")]
        result = _sync.apply_recording_filters(recordings, (), None)
        assert result == recordings
...
def test_download_file_streams_response_in_chunks(
    ...
    with tempfile.TemporaryDirectory() as destination:
        ...
        assert downloaded is True
        assert all(s == _sync.DOWNLOAD_CHUNK_SIZE for s in read_sizes)
...
def test_structured_log_formatter_outputs_json_with_extra_fields() -> None:
    ...
    formatter = _sync.StructuredLogFormatter()
...
def test_parse_pushgateway_url(value: str, expected: str) -> None:
    assert _metrics.parse_pushgateway_url(value) == expected
...
def test_metrics_state_round_trip() -> None:
    ...
    with tempfile.TemporaryDirectory() as destination:
        ...
        _metrics.save_metrics_state(state_file, metrics)

        assert _metrics.load_metrics_state(state_file) == 123.0
...
def test_metrics_state_missing_or_corrupt_returns_none() -> None:
    ...
    with tempfile.TemporaryDirectory() as destination:
        ...
        assert _metrics.load_metrics_state(missing_state_file) is None
        assert _metrics.load_metrics_state(corrupt_state_file) is None
...
def test_classify_run_failure(error: BaseException, expected: str) -> None:
    assert _metrics.classify_run_failure(error) == expected
...
def test_render_metrics_uses_last_run_gauges() -> None:
    ...
    output = _metrics.render_metrics(metrics)
...
def test_write_metrics_file_replaces_target() -> None:
    ...
    with tempfile.TemporaryDirectory() as destination:
        ...
        _metrics.write_metrics_file(metrics_file, "first\n")
        _metrics.write_metrics_file(metrics_file, "second\n")
...
def test_pushgateway_metrics_url_quotes_grouping_values() -> None:
    ...
    assert (
        _metrics.get_pushgateway_metrics_url(
